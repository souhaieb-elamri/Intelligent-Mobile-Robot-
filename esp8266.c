...
void ESP_Config(void) {
    ...
    esp_delay(500);
    SendString_USART3("AT+CWJAP=\"" WIFI_SSID "\",\"" WIFI_PASSWORD "\"\r\n");
    esp_delay(7000);
...
void sendDataToCloud(char* http) {
    ...
    SendATCommand("AT+CIPSTART=\"TCP\",\"" THINGSPEAK_HOST "\",80");
    esp_delay(3000);
...
void sendDataToNodeRED(char* json_payload) {
    ...
    SendATCommand("AT+CIPSTART=\"TCP\",\"" NODERED_IP "\"," NODERED_PORT);
    esp_delay(3000);

    sprintf(post_req,
        "POST /stm32-data HTTP/1.1\r\n"
        "Host: " NODERED_IP ":" NODERED_PORT "\r\n"
